...
```bash
npm install @epcc-sdk/permissions
# or
pnpm install @epcc-sdk/permissions
# or
yarn add @epcc-sdk/permissions
```
...
```ts
import { client } from "@epcc-sdk/permissions";
...
```ts
import { createClient } from "@epcc-sdk/permissions";
...
You can also pass this instance to any SDK function through the client option. This will override the default instance from `import { client } from "@epcc-sdk/permissions>".

```ts
const response = await getABuiltInRole({
    client: myClient,
});
```
...
```ts
import { getABuiltInRole } from "@epcc-sdk/permissions";

const product = await getABuiltInRole({
...
- **`listBuiltInRoles`** (`GET /v2/permissions/built-in-roles`)

- **`getABuiltInRole`** (`GET /v2/permissions/built-in-roles/{built_in_role_id}`)

- **`listCustomApiRolePolicies`** (`GET /v2/permissions/custom-api-role-policies`)

- **`createACustomApiRolePolicy`** (`POST /v2/permissions/custom-api-role-policies`)

- **`deleteACustomApiRolePolicy`** (`DELETE /v2/permissions/custom-api-role-policies/{custom_api_role_policy_id}`)
