# @epcc-sdk/sdks-application-keys SDK
...
```bash
npm install @epcc-sdk/sdks-application-keys
# or
pnpm install @epcc-sdk/sdks-application-keys
# or
yarn add @epcc-sdk/sdks-application-keys
```
...
```ts
import { client } from "@epcc-sdk/sdks-application-keys";
...
```ts
import { createClient } from "@epcc-sdk/sdks-application-keys";
...
You can also pass this instance to any SDK function through the client option. This will override the default instance from `import { client } from "@epcc-sdk/sdks-application-keys>".
...
```ts
import { getKey } from "@epcc-sdk/sdks-application-keys";

const product = await getKey({
...
- **`getAllKeys`** (`GET /v2/application-keys`)

- **`createKey`** (`POST /v2/application-keys`)

- **`deleteKey`** (`DELETE /v2/application-keys/{application_key_id}`)

- **`getKey`** (`GET /v2/application-keys/{application_key_id}`)

- **`updateKey`** (`PUT /v2/application-keys/{application_key_id}`)
