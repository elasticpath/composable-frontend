...
```bash
npm install @epcc-sdk/sdks-merchant-realm-mapping
# or
pnpm install @epcc-sdk/sdks-merchant-realm-mapping
# or
yarn add @epcc-sdk/sdks-merchant-realm-mapping
```
...
```ts
import { client } from "@epcc-sdk/sdks-merchant-realm-mapping";
...
```ts
import { createClient } from "@epcc-sdk/sdks-merchant-realm-mapping";
...
You can also pass this instance to any SDK function through the client option. This will override the default instance from `import { client } from "@epcc-sdk/sdks-merchant-realm-mapping>".

```ts
const response = await getMerchantRealmMapping({
    client: myClient,
});
```
...
```ts
import { getMerchantRealmMapping } from "@epcc-sdk/sdks-merchant-realm-mapping";

const product = await getMerchantRealmMapping({
...
- **`getMerchantRealmMappings`** (`GET /v2/merchant-realm-mappings`)
