...
```bash
npm install @epcc-sdk/integrations
# or
pnpm install @epcc-sdk/integrations
# or
yarn add @epcc-sdk/integrations
```
...
```ts
import { createClient } from "@epcc-sdk/integrations";
...
You can also pass this instance to any SDK function through the client option. This will override the default instance from `import { client } from "@epcc-sdk/integrations>".
...
```ts
import { getIntegration } from "@epcc-sdk/integrations";

const product = await getIntegration({
...
- **`deleteIntegration`** (`DELETE /integrations/{integration_uuid}`)

- **`getIntegration`** (`GET /integrations/{integration_uuid}`)

- **`updateIntegration`** (`PUT /integrations/{integration_uuid}`)

- **`listStoreLogs`** (`GET /integrations/logs`)

- **`listIntegrationLogs`** (`GET /integrations/{integration_uuid}/logs`)
