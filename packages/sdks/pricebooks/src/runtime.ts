...
export type PricebooksClientOptions = ConfiguredClientOptions<Config>

/**
 * A pricebooks client with a token source, the `auth` hook and a `fetch` that
 * refreshes on a 401 and backs off on a 429 already wired.
 *
 ...
 */
export function createPricebooksClient(options: PricebooksClientOptions): Client {
