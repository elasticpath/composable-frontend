...
```bash
npm install @epcc-sdk/sdks-currencies
# or
pnpm install @epcc-sdk/sdks-currencies
# or
yarn add @epcc-sdk/sdks-currencies
```
...
```ts
import { client } from "@epcc-sdk/sdks-currencies";
...
```ts
import { createClient } from "@epcc-sdk/sdks-currencies";
...
You can also pass this instance to any SDK function through the client option. This will override the default instance from `import { client } from "@epcc-sdk/sdks-currencies>".

```ts
const response = await getACurrency({
    client: myClient,
});
```
...
```ts
import { getACurrency } from "@epcc-sdk/sdks-currencies";

const product = await getACurrency({
...
- **`createACurrency`** (`POST /v2/currencies`)

- **`deleteACurrency`** (`DELETE /v2/currencies/{currencyID}`)

- **`getACurrency`** (`GET /v2/currencies/{currencyID}`)

- **`updateACurrency`** (`PUT /v2/currencies/{currencyID}`)
