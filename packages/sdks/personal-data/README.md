...
```bash
npm install @epcc-sdk/personal-data
# or
pnpm install @epcc-sdk/personal-data
# or
yarn add @epcc-sdk/personal-data
```
...
```ts
import { client } from "@epcc-sdk/personal-data";
...
```ts
import { createClient } from "@epcc-sdk/personal-data";
...
You can also pass this instance to any SDK function through the client option. This will override the default instance from `import { client } from "@epcc-sdk/personal-data>".

```ts
const response = await getPersonalDataLogs({
    client: myClient,
});
```
...
```ts
import { getPersonalDataLogs } from "@epcc-sdk/personal-data";

const product = await getPersonalDataLogs({
...
- **`getPersonalDataLogs`** (`GET /v2/personal-data/logs`)

- **`getRelatedDataEntries`** (`GET /v2/personal-data/related-data-entries`)

- **`getErasureRequests`** (`GET /v2/personal-data/erasure-requests`)

- **`postErasureRequest`** (`POST /v2/personal-data/erasure-requests`)

- **`getErasureRequest`** (`GET /v2/personal-data/erasure-requests/{erasure_request_id}`)

- **`getLogsTtl`** (`GET /v2/settings/logs-ttl`)

- **`putLogsTtl`** (`PUT /v2/settings/logs-ttl`)
