...
class DemoRecorder {
  ...
  constructor() {
    ...
    this.command = ["ep", ...process.argv.slice(2)].join(" ")
  }
  ...
  addOrUpdateConcurrentOutput({
    ...
    const { processes } = last!.properties as {
  ...
  withFormattedConcurrent(recorded: Event[]) {
    return recorded.map((event) => {
      if (event.type === "concurrent") {
        ...
        const formatted = processes.map(
          ({ prefix, steps }: { prefix: string; steps: ConcurrencyStep[] }) => {
        ...
        return {
          type: "concurrent",
          properties: { footer, processes: formatted },
        }
      }
      return event
    })
