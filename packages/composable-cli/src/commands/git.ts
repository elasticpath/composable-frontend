import { hasGit } from "./local"
import { appendFileSync } from "./fs"
import { AbortError } from "./error"
import { cwd } from "./path"
import { outputContent, outputToken, outputDebug } from "./output"
...
export async function initializeGitRepository(
  ...
  outputDebug(
    outputContent`Initializing git repository at ${outputToken.path(
      directory,
    )}...`,
  )
 ...
 */
export async function checkIfIgnoredInGitRepository(
  directory: string,
  files: string[],
): Promise<string[]> {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
...
export function createGitIgnore(
  ...
  outputDebug(
    outputContent`Creating .gitignore at ${outputToken.path(directory)}...`,
  )
...
export async function getLatestGitCommit(
  directory?: string,
): Promise<DefaultLogFields & ListLogLine> {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
 ...
 */
export async function addAllToGitFromDirectory(
  directory?: string,
): Promise<void> {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
...
export async function createGitCommit(
  ...
  const commitOptions = options?.author
...
export async function getHeadSymbolicRef(directory?: string): Promise<string> {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const ref = await git({ baseDir: directory }).raw(
    "symbolic-ref",
    "-q",
    "HEAD",
  )
...
export async function ensureInsideGitDirectory(
  ...
  if (!(await git({ baseDir: directory }).checkIsRepo())) {
    throw new OutsideGitDirectoryError(
      `${outputToken.path(directory || cwd())} is not a Git directory`,
    )
  }
}
...
export async function ensureIsClean(directory?: string): Promise<void> {
  if (!(await isClean(directory))) {
    throw new GitDirectoryNotCleanError(
      `${outputToken.path(directory || cwd())} is not a clean Git directory`,
    )
  }
}
