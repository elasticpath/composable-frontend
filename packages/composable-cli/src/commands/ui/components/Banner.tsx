...
import { Link, LinksContext } from "../contexts/LinksContext"
  ...
  return (
    <LinksContext.Provider
      value={{
        links,
        addLink: (label, url) => {
          const id: string | undefined = Object.keys(links.current).find(
            (id) => links.current[id]!.url === url,
          )
          if (id) {
            return id
          }
    ...
    >
      ...
      >
        ...
        {/* TODO: gap is supported in ink 4 but not ink 3 */}
...
const BoxWithTopBottomLines: FunctionComponent<BannerProps> = ({
  type,
  children,
}) => {
  const { twoThirds } = useLayout()
...
const Banner: FunctionComponent<BannerProps> = ({ ...props }) => {
  if (props.type === "external_error") {
    return React.createElement(BoxWithTopBottomLines, props, props.children)
  } else {
    return React.createElement(BoxWithBorder, props, props.children)
  }
}
