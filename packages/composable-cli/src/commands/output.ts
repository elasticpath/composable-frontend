import { isUnitTest, isVerbose } from "./local"
import { PackageManager } from "./node-package-manager"
...
export const outputToken = {
  ...
  cyan(value: OutputMessage): ColorContentToken {
    return new ColorContentToken(value, chalk.cyan)
  },
  yellow(value: OutputMessage): ColorContentToken {
    return new ColorContentToken(value, chalk.yellow)
  },
  magenta(value: OutputMessage): ColorContentToken {
    return new ColorContentToken(value, chalk.magenta)
  },
  green(value: OutputMessage): ColorContentToken {
    return new ColorContentToken(value, chalk.green)
  },
  packagejsonScript(
...
export function outputSuccess(
  ...
  const message = chalk.bold(`✅ Success! ${stringifyMessage(content)}.`)
 ...
 * @param logger - The logging function to use to output to the user.
 */
export function outputCompleted(
  content: OutputMessage,
  logger: Logger = consoleLog,
): void {
  const message = `${chalk.green("✔")} ${stringifyMessage(content)}`
...
export function outputDebug(
  ...
  const message = chalk.gray(stringifyMessage(content))
...
export function outputWarn(
  ...
  const message = chalk.yellow(stringifyMessage(content))
 ...
 * @returns True if the console outputs should display chalk, false otherwise.
 */
...
export function getOutputUpdateCLIReminder(
  ...
  const updateCommand = outputToken.packagejsonScript(
    packageManager,
    "ep upgrade",
  )
