...
export function createGenerateBuilder(yargs: yargs.Argv<{}>) {
  return yargs
...
export function createGenerateCommand(
  ...
  return async function (args: GenerateCommandArgs) {
    ...
    const { cliOptions, schematicOptions, _, schematic } = parseArgs(args)
    ...
    if (cliOptions.interactive) {
      ...
      if (creds) {
        ...
        // TODO: get application keys
        // TODO: work out the most secure way to store the application keys (client secret)

        const { data } = await createApplicationKeys(apiUrl, token, "cli") // TODO: name should be user provided project name plus new date
...
function parseArgs(args: GenerateCommandArgs): Options {
  ...
  const { _, $0, schematic = null, ...options } = args
