...
export class SearchResponseAdapter {
  ...
  typesenseResponse: any
  instantsearchRequest: SearchRequest
  configuration: Configuration
  // allTypesenseResults: MultiSearchResponse["results"]
  ...
  constructor(
    ...
    configuration: Configuration,
    allTypesenseResults: MultiSearchResponse["results"] = [],
    fullTypesenseResponse: MultiSearchResponse = {} as MultiSearchResponse,
  ) {
  ...
  _adaptHits(typesenseHits: TypesenseHit[]): AdaptedHit[] {
    ...
    adaptedResult = typesenseHits.map((typesenseHit) => {
      ...
      if (this.fullTypesenseResponse.conversation) {
        adaptedHit._rawTypesenseConversation =
          this.fullTypesenseResponse.conversation
      }
      ...
      ;[
        ...
        if (
          Object.keys(typesenseHit).includes(metadataField) &&
          !Object.keys(adaptedHit).includes(metadataField)
        ) {
          // @ts-ignore
  ...
  _adaptHighlightResult(
    ...
    if (
      typesenseHit.highlight != null &&
      this.isHighlightPost0240RCN32Format(typesenseHit.highlight)
    ) {
  ...
  adaptHighlightsArray(
    ...
    Object.entries(result).forEach(([k, v]: [string, any]) => {
      ...
      if (value == null) {
        ...
        result[attribute] = []
        typesenseHit.document[attribute].forEach(
          (unhighlightedValueFromArray: any, index: number) => {
        ...
        // Side note: Typesense does not support highlights for nested objects in this `highlights` array,
        //  so we pass in an empty object below
        result[attribute] = this._adaptHighlightInObjectValue(
          value,
          {},
          snippetOrValue,
        )
    ...
    return Object.assign(
      ...
      ...Object.entries(objectValue).map(([attribute, value]) => {
        ...
        if (value == null) {
          ...
          adaptedValue = this._adaptHighlightInPrimitiveValue(
            value,
            highlightObjectValue?.[attribute],
            snippetOrValue,
          )
  ...
  _adaptHighlightInPrimitiveValue(
    ...
    if (highlightPrimitiveValue != null) {
      return {
        ...
        matchLevel:
          (highlightPrimitiveValue.matched_tokens || []).length > 0
            ? "full"
            : "none",
  ...
  _adaptFacets(typesenseFacetCounts: TypesenseFacetCount[] | undefined): {
  ...
  _adaptFacetStats(typesenseFacetCounts: TypesenseFacetCount[] | undefined): {
  ...
  _adaptRenderingContent(typesenseFacetCounts: TypesenseFacetCount[]): any {
    const adaptedResult: any = Object.assign(
      {},
      this.configuration.renderingContent,
    )
          ...
          (Array.isArray(typesenseFacetCounts) ? typesenseFacetCounts : [])
            ...
            .concat(
              this.allTypesenseResults
                .map((r: any) => r.facet_counts || [])
                .flat()
                .map((fc: any) => fc["field_name"])
                .filter((f: any) => f),
            ),
        ),
      ]
  ...
  adapt(): AdaptedSearchResponse {
    ...
    const adaptedResult: AdaptedSearchResponse = {
      hits: this.typesenseResponse.grouped_hits
        ...
        : this._adaptHits(this.typesenseResponse.hits as TypesenseHit[]),
    ...
    if (this.typesenseResponse.metadata) {
      ...
      adaptedResult.userData = this._adaptUserData(
        this.typesenseResponse.metadata,
      )
    }
