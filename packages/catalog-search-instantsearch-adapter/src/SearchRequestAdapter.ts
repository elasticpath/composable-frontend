...
import type { Configuration, SearchParameters, CollectionSpecificSearchParameters } from "./Configuration";
...
interface TypesenseClient {
  multiSearch: {
    perform: (searchRequest: { searches: TypesenseSearchParams[]; union?: any }, commonParams?: any) => Promise<any>;
  };
}
...
export class SearchRequestAdapter {
  ...
  constructor(instantsearchRequests: InstantSearchRequest[], typesenseClient: TypesenseClient, configuration: Configuration) {
  ...
  _shouldUseExactMatchForField(fieldName: string, collectionName: string): boolean {
    if (
  ...
  _adaptFacetFilters(facetFilters: Array<string | string[]> | undefined, collectionName: string): string {
    let adaptedResult = "";
  ...
  _parseFacetFilter(facetFilter: string): ParsedFacetFilter {
    let filterStringMatchingRegex: RegExp, facetFilterMatches: RegExpMatchArray | null, fieldName: string, fieldValue: string;
  ...
  _adaptNumericFilters(numericFilters: string[] | undefined): string {
  ...
  _parseNumericFilter(numericFilter: string): ParsedNumericFilter {
    let filterStringMatchingRegex: RegExp, numericFilterMatches: RegExpMatchArray | null;
    let fieldName: string, operator: string, fieldValue: string;
  ...
  _adaptGeoFilter({ insideBoundingBox, aroundRadius, aroundLatLng, insidePolygon }: Pick<InstantSearchParams, 'insideBoundingBox' | 'aroundRadius' | 'aroundLatLng' | 'insidePolygon'>): string | undefined {
  ...
  _adaptFilters(instantsearchParams: InstantSearchParams, collectionName: string): string {
  ...
  _adaptIndexName(indexName: string): string {
    const match = indexName.match(SearchRequestAdapter.INDEX_NAME_MATCHING_REGEX);
  ...
  _adaptFacetBy(facets: string | string[] | undefined, collectionName: string): string {
    return [facets]
      .flat()
  ...
  _buildSearchParameters(instantsearchRequest: InstantSearchRequest): TypesenseSearchParams {
    ...
    const typesenseSearchParams: TypesenseSearchParams = Object.assign({}, snakeCasedAdditionalSearchParameters) as TypesenseSearchParams;
  ...
  async request(): Promise<any> {
    ...
    const searchRequest: { searches: TypesenseSearchParams[]; union?: any } = { searches: searches };
