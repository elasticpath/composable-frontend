#!/usr/bin/env node
/**
 * PROTOTYPE — throwaway. Answers ticket 04 of the spec-sync map; not production code.
 *
 * Takes a spec that has landed under packages/sdks/specs/upstream/, refreshes the
...
if (row.divergence !== "none") {
  fail(
    `"${specKey}" is marked divergence="${row.divergence}"${row.note ? ` (${row.note})` : ""}. ` +
      `Refreshing it needs a rule this prototype does not have. Left untouched.`,
    3,
  )
}
...
if (!existsSync(upstreamPath)) fail(`nothing landed at specs/upstream/${row.upstream}`, 2)
...
// The generated READMEs churn on every build and are reverted in release.yml; do the same
// here so a spec PR carries the spec and the client, nothing else.
sh("git", ["restore", "--staged", "--worktree", ":(icase,glob)**/readme.md"], { stdio: "inherit" })
